...
*/
    ...
    public static String addDay(String s, String property) throws java.text.ParseException{
    	return addDays(s, property, "yyyy/MM/dd");
	}
    /**
     * return add day to date strings
     * @param String date string
    ...
    public static String addDays(String s, String day, String format) throws java.text.ParseException{
        java.text.SimpleDateFormat formatter =
            new java.text.SimpleDateFormat (format, java.util.Locale.KOREA);
        java.util.Date date = check(s, format);

        date.setTime(date.getTime() + (Long.parseLong(day) * 1000 * 60 * 60 * 24));
        return formatter.format(date);
    }
    /**
     * return add day to date strings with user defined format.
...
//        if (((toDay - fromDay) < 0) ) result += fromDate.compareTo(toDate);
	 ...
	 */
	 ...
	 /**
	  * 오늘 일자를 12자리(년월일시분:yyyyMMddHHmm)의 문자열로 돌려준다.
	  * @return
	  */
	 ...
	 public static String getFastFormat(Date date)
	 {
		 if(date == null)
			 return "";

 		Format fdf = FastDateFormat.getInstance("yyyy/MM/dd");
 		return fdf.format(date);
	 }
	 
	 
	 /**
	  * Get date of java.util.Date type.
	 * @param year
	 * @param month
	 * @param date
	 * @param hour
	...
	public static Date getDate(int year, int month, int date, int hour, int min, int sec) {
	     ...
	     calendar.set(year, month-1, date, hour, min, sec);    // 0=JANUARY, 1=FEBRUARY, 11=DECEMBER
	     return new Date(calendar.getTimeInMillis());
	 }
	 
	 
	 /**
	  * (30/04/2015, 01/04/2016) => 12 
	 * @param bgnDt
	 * @param endDt
	 * @return
	 */
	public static int getMonthsDifference(Calendar bgnDt, Calendar endDt) {
	     
	     int diffYear = endDt.get(Calendar.YEAR) - bgnDt.get(Calendar.YEAR);
	     int diffMonth = diffYear * 12 + endDt.get(Calendar.MONTH) - bgnDt.get(Calendar.MONTH);
	     return diffMonth;
	 }
      
	 
	 public static int getMonthsDifference(Date bgnDt, Date endDt) {
	     
	     Calendar bgnCalendar = dateToCalendar(bgnDt);
	     Calendar endCalendar = dateToCalendar(endDt);
	     return getMonthsDifference(bgnCalendar, endCalendar);
     }
     ...
     */
    public static String reformat(String dateString, String srcFmt, String dstFmt) {
        if (StringUtils.isBlank(dateString)) return dateString;
        ...
        try {
            result = dstSdf.format(srcSdf.parse(dateString));
        } catch (ParseException e) {
            throw new RuntimeException("Date reformat failed");
        }
    ...
    public static Date hourMinuteToDate(Date eventDate, int hour, int minute) {
    ...
    //example 11:00 to Date
    public static Date strHourMinuteToDate(Date eventDate, String hourMinute) {
        String[] arrTime = hourMinute.split ( ":" );
        int hour = Integer.parseInt ( arrTime[0] );
        int minute = Integer.parseInt ( arrTime[1] );
        return hourMinuteToDate(eventDate, hour, minute);
    }
    ...
    public static boolean isDateInWeek(Date date, int firstDayOfWeek) {
        Calendar currentCalendar = Calendar.getInstance();
        currentCalendar.setFirstDayOfWeek(firstDayOfWeek);
        int week = currentCalendar.get(Calendar.WEEK_OF_YEAR);
        int year = currentCalendar.get(Calendar.YEAR);
        Calendar targetCalendar = Calendar.getInstance();
        targetCalendar.setFirstDayOfWeek(firstDayOfWeek);
    ...
    public static String dateStringFormatConversion(String fromFormatDate, String toFormatDate, String dateStr){
    	DateFormat dfFrm = new SimpleDateFormat(fromFormatDate);
    	DateFormat dfTo = new SimpleDateFormat(toFormatDate);
    	String convertedDate = "";
    	try {
			Date frmDate = dfFrm.parse(dateStr);
			convertedDate = dfTo.format(frmDate);
		} catch (ParseException e) {
			e.printStackTrace();
		}
